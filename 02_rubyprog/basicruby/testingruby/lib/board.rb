class Board
	...
	def display_choice_separator(width = HOLE_WIDTH, new_line = true, tab = true)
		display_separator_row(PIECES[:vertical], PIECES[:vertical], PIECES[:vertical], width, new_line, tab, lastrow = true)
	end

	def display_separator_row(left, middle, right, width, new_line, tab, lastrow = false)
		print "\t" if tab
		to_display = left
		COLUMNS.times do
	...
	def store_choice(picked_column)
		ROWS_CHECKER.each do |key|
			ROWS_CHECKER[key] = 'O' if ROWS_CHECKER[key].nil?
		end
	end
